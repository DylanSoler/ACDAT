...
AS
	BEGIN
		...
		BEGIN
			UPDATE SORTEO
				SET CATEGORIA_5 = (SELECT CATEGORIA_5_ACERTANTES FROM SORTEO WHERE ID=@IDSorteo) * 8
			WHERE ID=@IDSorteo

			DECLARE @Cuantia45 money = (SELECT RECAUDACION FROM SORTEO WHERE ID = @IDSorteo) * 0.45
			SELECT @Cuantia45 -= (SELECT CATEGORIA_5_ACERTANTES FROM SORTEO WHERE ID=@IDSorteo) * 8
...
/* 
	Procedimiento que asigna el numero de acertantes por categorias de un sorteo
*/
GO
...
AS
	BEGIN
		...
		UPDATE SORTEO
			...
			/* UPDATES de la tabla de las apuestas multiples */			
			DECLARE @SUMA int = 0
...
--SELECT @SUMA,
--		CASE
--			WHEN NACERTADOS=5 AND CANTIDADNUMEROS=5 THEN @SUMA+1
--			WHEN NACERTADOS=5 AND COMPLEMENTARIOACERTADO=1 THEN @SUMA+1
--			WHEN NACERTADOS=4 AND COMPLEMENTARIOACERTADO=1 AND CANTIDADNUMEROS=5 THEN @SUMA+2
--			WHEN NACERTADOS=6 THEN @SUMA+6
--		END
